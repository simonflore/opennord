...
- [~] **Interpretation layer — 202 params DONE (enum + numeric).** Generated by exhaustively evaluating ns4decode's real interpreter over every raw value for each pure (non-dependent) parameter (`values.generated.ts`), validated against the fixture (423/423 values match, 0 mismatch). Covers volume→dB, pan, pitch (cents/semi), envelope amounts, transpose, and all simple enums. Remaining: **dependent** interpreters — morphs ("with wheel/A.T./pedal"), synth category/wave (depend on type+version), timbre, drawbars, rate-vs-master-clock, frequency/time curves on wide fields, and sample-ID→name. These need the dependency context wired at decode time.
