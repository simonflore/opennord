...
describe.skipIf(!existsSync(FIXTURE_DIR))('decodeNl4 (nl4s — Sound)', () => {
  ...
  it('exposes correctly-sized nl4s clusters', () => {
    ...
    expect(prog.voice?._packed).toHaveLength(216);
    expect(prog.fxArp?._packed).toHaveLength(41);
  });

  it('decodes the voice head selector within its observed range (0-10) across the corpus', () => {
    for (const name of nl4sFiles()) {
      const prog = decodeNl4(load(name));
      expect(prog.voice, name).toBeDefined();
      expect(prog.voice!.mode, name).toBeGreaterThanOrEqual(0);
      expect(prog.voice!.mode, name).toBeLessThanOrEqual(10);
    }
  });
  ...
  it('decodes the byte-aligned fields within their observed corpus ranges', () => {
    for (const name of nl4sFiles()) {
      const prog = decodeNl4(load(name));
      ...
      expect(prog.fxArp!.modeB, name).toBeLessThanOrEqual(20);
    }
  });
...
describe.skipIf(!existsSync(FIXTURE_DIR))('decodeNl4 (nl4p — Program)', () => {
  ...
  it('exposes four slot blocks at the 315-byte stride', () => {
    const prog = decodeNl4(load(nl4pFiles()[0]));
    expect(prog._slot0).toHaveLength(296);
    expect(prog._slot1).toHaveLength(307);
    expect(prog._slot2).toHaveLength(307);
    expect(prog._slot3).toHaveLength(307);
  });
